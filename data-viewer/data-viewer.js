...
class TwitterDataViewer {
  ...
  createTweetCard(tweet) {
    ...
    // Add pinned badge if this is a pinned tweet
    if (tweet.is_pinned) {
      const pinnedBadge = document.createElement("span");
      pinnedBadge.className = "pinned-badge";
      pinnedBadge.innerHTML = "📌 Pinned";
      pinnedBadge.style.cssText = `
        background: #fff3cd;
        color: #856404;
      ...
      // Insert the pinned badge after the source badge
      sourceBadge.parentNode.insertBefore(pinnedBadge, sourceBadge.nextSibling);
    }
    ...
    if (tweet.quoted_tweet) {
      const quotedTweetContainer = this.createQuotedTweetCard(tweet.quoted_tweet);
      tweetText.appendChild(quotedTweetContainer);
    }
  ...
  createQuotedTweetCard(quotedTweet) {
    ...
    // Author avatar
    if (quotedTweet.author_profile_image_url) {
      const avatarLink = document.createElement("a");
      avatarLink.href = `https://x.com/${quotedTweet.author_screen_name || "unknown"}`;
      avatarLink.target = "_blank";
      avatarLink.className = "quoted-avatar-link";

      const avatar = document.createElement("img");
      avatar.src = quotedTweet.author_profile_image_url;
      avatar.alt = quotedTweet.author_name || "Unknown";
    ...
    // Author name and handle container
    ...
    authorNameLink.href = `https://x.com/${quotedTweet.author_screen_name || "unknown"}`;
    authorNameLink.target = "_blank";
    authorNameLink.className = "quoted-author-name-link";
    authorNameLink.textContent = quotedTweet.author_name || "Unknown User";

    const authorHandle = document.createElement("span");
    authorHandle.className = "quoted-author-handle";
    authorHandle.textContent = `@${quotedTweet.author_screen_name || "unknown"}`;

    // Timestamp if available
    const timestamp = document.createElement("span");
    timestamp.className = "quoted-timestamp";
    if (quotedTweet.created_at) {
      timestamp.textContent = this.formatDate(quotedTweet.created_at);
    }
    ...
    quoteText.innerHTML = this.processTextLinks(quotedTweet.text || "");

    // Media content for quoted tweet
    const quotedMedia = document.createElement("div");
    quotedMedia.className = "quoted-media";
    if (quotedTweet.has_media && quotedTweet.media_info && quotedTweet.media_info.length > 0) {
      this.renderTweetMedia(quotedMedia, quotedTweet.media_info, quotedTweet);
      quotedMedia.style.display = "block";
    }

    // Footer with engagement stats and link
    ...
    if (quotedTweet.like_count > 0 || quotedTweet.retweet_count > 0 || quotedTweet.reply_count > 0) {
      if (quotedTweet.reply_count > 0) {
        const replies = document.createElement("span");
        replies.className = "quoted-stat";
        replies.innerHTML = `<svg viewBox="0 0 24 24" class="quoted-stat-icon"><path d="M1.751 10c0-4.42 3.584-8 8.005-8h4.366c4.49 0 8.129 3.64 8.129 8.13 0 2.96-1.607 5.68-4.196 7.11l-8.054 4.46v-3.69h-.067c-4.49.1-8.183-3.51-8.183-8.01z"></path></svg>${this.formatNumber(quotedTweet.reply_count)}`;
        statsContainer.appendChild(replies);
      }
      
      if (quotedTweet.retweet_count > 0) {
        const retweets = document.createElement("span");
        retweets.className = "quoted-stat";
        retweets.innerHTML = `<svg viewBox="0 0 24 24" class="quoted-stat-icon"><path d="M4.75 3.79l4.603 4.3-1.706 1.82L6 8.38v7.37c0 .97.784 1.75 1.75 1.75H13V20H7.75c-2.347 0-4.25-1.9-4.25-4.25V8.38L1.853 9.91.147 8.09l4.603-4.3zm11.5 2.71H11V4h5.25c2.347 0 4.25 1.9 4.25 4.25v7.37l1.647-1.53 1.706 1.82-4.603 4.3-4.603-4.3 1.706-1.82L18 15.62V8.25c0-.97-.784-1.75-1.75-1.75z"></path></svg>${this.formatNumber(quotedTweet.retweet_count)}`;
        statsContainer.appendChild(retweets);
      }
      
      if (quotedTweet.like_count > 0) {
        const likes = document.createElement("span");
        likes.className = "quoted-stat";
        likes.innerHTML = `<svg viewBox="0 0 24 24" class="quoted-stat-icon"><path d="M20.884 13.19c-1.351 2.48-4.001 5.12-8.379 7.67l-.503.3-.504-.3c-4.379-2.55-7.029-5.19-8.382-7.67-1.36-2.5-1.41-4.86-.514-6.67.887-1.79 2.647-2.91 4.601-3.01 1.651-.09 3.368.56 4.798 2.01 1.429-1.45 3.146-2.1 4.796-2.01 1.954.1 3.714 1.22 4.601 3.01.896 1.81.846 4.17-.514 6.67z"></path></svg>${this.formatNumber(quotedTweet.like_count)}`;
    ...
    // View original link
    if (quotedTweet.tweet_url) {
      const viewLink = document.createElement("a");
      viewLink.href = quotedTweet.tweet_url;
      viewLink.target = "_blank";
      viewLink.className = "quoted-view-link";
      viewLink.innerHTML = `<svg viewBox="0 0 24 24" class="quoted-link-icon"><path d="M18.36 5.64c-1.95-1.96-5.11-1.96-7.07 0L9.88 7.05 8.46 5.64l1.42-1.42c2.73-2.73 7.16-2.73 9.9 0 2.73 2.74 2.73 7.17 0 9.9l-1.42 1.42-1.41-1.42 1.41-1.41c1.96-1.96 1.96-5.12 0-7.07z"></path><path d="m15.54 18.36-1.41 1.42c-2.73 2.73-7.16 2.73-9.9 0-2.73-2.74-2.73-7.17 0-9.9l1.42-1.42 1.41 1.42-1.41 1.41c-1.96 1.96-1.96 5.12 0 7.07 1.95 1.96 5.11 1.96 7.07 0l1.41-1.42 1.41 1.42z"></path><path d="m14.12 11.46-4.24 4.24-1.42-1.42 4.24-4.24 1.42 1.42z"></path></svg>View on X`;
      footerSection.appendChild(viewLink);
    }

    // Assemble the quoted tweet
